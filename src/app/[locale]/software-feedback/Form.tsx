...
import { CheckCircle, LoadingSpinner, XCircle } from "./Icons";
...
export const Form = () => {
	...
	const mutation = useMutation({
		mutationFn: async (data: CreateChituboxDentalUserFeedbackDto) => {
			setStatus(FormStatus.UPLOADING_FORM);
			const feedback = await dentalUserFeedback(data);
			if (!feedback.id) {
				throw new Error(feedback.message);
			}
			setStatus(FormStatus.UPLOADING_ATTACHMENTS);
	...
	switch (status) {
		...
		case FormStatus.UPLOADING_FORM:
			return (
				<div
					className="flex-auto flex justify-center items-center lg:w-[884px] p-12 gap-3
					bg-white
					rounded-[16px]"
				>
					<LoadingSpinner size={24} />
					<p className="text-gray-500">
						{t("form-uploading-message")}
					</p>
				</div>
			);
		case FormStatus.UPLOADING_ATTACHMENTS:
			return (
				<div
					className="flex-auto flex justify-center items-center lg:w-[884px] p-12 gap-3
					bg-white
				...
				>
					<LoadingSpinner size={24} />
					<p className="text-gray-500">
						{t("form-uploading-attachments-message")}
					</p>
				</div>
			);
		case FormStatus.SUCCESS:
			return (
				...
				>
					<CheckCircle size={24} />
					<p className="text-gray-500">{t("form-success-message")}</p>
				</div>
			);
